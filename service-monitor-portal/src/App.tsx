...
const ACCOUNTS = ["NW", "BB", "RBS", "VQS", "MONZO", "FAB"];
...
const Dashboard = () => {
  ...
  const rawServices = services.length > 0 ? services : (showExamples ? exampleServices : []);

  // Compute filtered services
  const displayServices = rawServices.filter(service => {
    ...
    return matchAccount && matchEnv && matchServiceType;
  });

  const hasActiveFilters = selectedAccount !== "" || selectedEnv !== "" || selectedServiceType !== "";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {displayServices.length === 0 ? (
          <div className="col-span-full py-12 text-center bg-white rounded-2xl border border-dashed border-slate-300">
            ...
            <p className="text-slate-500">No services match your active filters.</p>
            {hasActiveFilters && (
              <button 
                onClick={clearFilters}
